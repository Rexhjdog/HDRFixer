...
using ticks_t = timer_large_integer::type;
    ...
    DOCTEST_THREAD_LOCAL int g_no_colors_state = 0; // 0=default, 1=force true, 2=force false
    inline bool get_no_colors() {
        if (g_no_colors_state == 1) return true;
        if (g_no_colors_state == 2) return false;
        return g_cs && g_cs->no_colors;
    }
