    ...
    struct JUnitReporter : public IReporter
    {
        ...
        void test_run_end(const TestRunStats& p) override {
            ...
            for(const auto& testCase : testCaseData.testcases) {
                ...
                if(!testCase.system_out.empty())
                    xml.scopedElement("system-out").writeText(testCase.system_out);
                if(!testCase.system_err.empty())
                    xml.scopedElement("system-err").writeText(testCase.system_err);
        ...
        void log_message(const MessageData& mb) override {
            if(mb.m_severity & assertType::is_warn) {
                ...
                log_contexts(os);
                if(!testCaseData.testcases.empty())
                    testCaseData.testcases.back().system_out += os.str();
                return;
            }
