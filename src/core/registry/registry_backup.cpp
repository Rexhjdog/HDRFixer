...
static std::string sanitize_filename(const std::string& name) {
    ...
    // Windows invalid: < > : " / \ | ? *
    // We also want to prevent control characters and path traversal dots
...
// Linux implementation (mock)
static std::string to_utf8(const std::wstring& wstr) {
    // Basic implementation for testing ASCII
    std::string result;
    for (wchar_t wc : wstr) {
...
static std::filesystem::path get_backup_directory() {
    ...
    if (home) {
        return std::filesystem::path(home) / ".config/HDRFixer/backups";
    }
    return std::filesystem::path("/tmp/HDRFixer/backups");
}
...
RegistryBackupManager::RegistryBackupManager(std::filesystem::path backup_dir)
...
std::expected<void, std::string> RegistryBackupManager::save_backup(const BackupSet& backup) {
    ...
    std::string safe_name = sanitize_filename(backup.name);

    // Build the file path
    std::filesystem::path file_path = backup_dir_ / (safe_name + ".bak");
...
std::expected<BackupSet, std::string> RegistryBackupManager::load_backup(const std::string& name) {
    // Sanitize input name to prevent traversal
    std::string safe_name = sanitize_filename(name);
    std::filesystem::path file_path = backup_dir_ / (safe_name + ".bak");
    ...
    BackupSet backup;
    backup.name = name; // Store original requested name? Or sanitized? Original for consistency.
